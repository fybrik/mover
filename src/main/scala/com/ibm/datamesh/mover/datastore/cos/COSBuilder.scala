...
import com.ibm.datamesh.mover.spark.{COSSourceServiceName, COSTargetServiceName}
...
case object COSBuilder extends DataStoreBuilder {
  private def createCOSDataStore(iType: InputType, config: Config): Try[COS] = {
    Try {
      ...
      COS(iType, endpoint, bucket, objectKey, fileFormat, region, apiKey, serviceInstance, accessKey, secretKey, serviceName)
    }
  }
